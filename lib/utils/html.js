...
*/
var html_escape = exports.escape = function (value) {
    return value
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
var linebreaks = exports.linebreaks = function (value, options) {
    ...
    if (options.onlybr) {
        return (options.escape ? html_escape(value) : value).replace(/\n/g, '<br />');
    }

    var lines = value.split(/\n{2,}/);
    if (options.escape) {
        lines = lines.map( function (x) { return '<p>' + html_escape(x).replace('\n', '<br />') + '</p>'; } );
    } else {
        ...
        if (match) {
            ...
            if (url) {
                ...
                if (options.escape) {
                    ...
                    trail = html_escape(trail);
                    url = html_escape(url);
                    trimmed = html_escape(trimmed);
                }
                middle = '<a href="' + url + '"' + nofollow + '>' + trimmed + '</a>';
                words[i] = lead + middle + trail;
            }
        } else if (options.escape) {
            words[i] = html_escape(word);
        }
    });
    return words.join('');
}
